...
export default class BookController {
  ...
  updateBookAvailability = async (req, res) => {
    ...
    const {quantity} = req.body.availableCopies
    console.log(quantity);
    const result = await this.bookRepository.updateBookAvailability(quantity)
    if(result){
      res.status(200).send("Update was successful")
    }else{
      res.status(404).send("Update was not successful")
    }
  };
